...
/// SYSCOIN: Both the committed backlog and recovered journal ranges are canonically sorted. Keep a
/// monotonic range cursor so startup coverage classification is O(backlog + journal ranges), not a
/// fresh O(journal ranges) scan for every unproved batch.
struct SortedJournalCoverageCursor<'a> {
    ranges: &'a [(u64, u64)],
    next_range: usize,
    last_batch: Option<u64>,
}

impl<'a> SortedJournalCoverageCursor<'a> {
    fn new(ranges: &'a [(u64, u64)]) -> Self {
        debug_assert!(ranges.iter().all(|(from, to)| from <= to));
    ...
    fn contains(&mut self, batch_number: u64) -> bool {
        debug_assert!(self.last_batch.is_none_or(|last| last < batch_number));
        self.last_batch = Some(batch_number);
        while self
            .ranges
            .get(self.next_range)
        ...
        self.ranges
            .get(self.next_range)
            .is_some_and(|(from, to)| *from <= batch_number && batch_number <= *to)
    }
}
...
impl SnarkProvingPipelineStep {
    ...
    async fn rehydrate_snark_queue(
        ...
        let mut journal_coverage = SortedJournalCoverageCursor::new(journal_covered_ranges);
        for batch_number in recovery_from..=last_committed_batch_number {
            if journal_coverage.contains(batch_number) {
                continue;
            }
...
mod tests {
    ...
    #[test]
    fn sorted_journal_coverage_cursor_advances_each_range_once() {
        let ranges = [(2, 3), (5, 7), (10, 10)];
        let mut cursor = SortedJournalCoverageCursor::new(&ranges);
        let covered: Vec<_> = (1..=11)
            .filter(|batch_number| cursor.contains(*batch_number))
            .collect();

        assert_eq!(covered, vec![2, 3, 5, 6, 7, 10]);
        assert_eq!(cursor.next_range, ranges.len());
    }
    ...
    async fn recovery_fixture(
        ...
        // SYSCOIN: CommittedBatchProvider authenticates canonical genesis on every startup,
        // including recovery fixtures whose executed frontier is already past batch zero.
        asserter.push_success(&Bytes::from(genesis_batch_info.hash().abi_encode()));
