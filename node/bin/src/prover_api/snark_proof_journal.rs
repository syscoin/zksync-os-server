...
use crate::prover_api::prover_job_map::MAX_STARTUP_RECOVERY_RANGES;
...
// record. Bound serialized output and restart disk input at 256 MiB; the typed decoder separately
// enforces every protocol-sized collection before canonical recovery can retain the record.
pub(crate) const MAX_JOURNAL_RECORD_BYTES: usize = 256 * 1024 * 1024;
// SYSCOIN: Explicitly budget the complete unconfirmed recovery set as well as each record. Eight
// GiB matches the generated production accepted-FRI disk-backlog default while leaving ample room
// for decoded recovery state on the 64-GiB prover hosts. At the ceiling, new wrapper persistence
// returns a retryable error until L1 confirmation retires older files; this deliberately trades
// proof throughput during an extended settlement outage for bounded restart memory.
const MAX_JOURNAL_TOTAL_BYTES: u64 = 8 * 1024 * 1024 * 1024;
...
impl DurableBatchMetadataV2 {
    fn into_batch_metadata(self) -> anyhow::Result<BatchMetadata> {
        ...
        anyhow::ensure!(
            self.batch_info.commit_info.operator_da_input.len()
                <= MAX_JOURNALED_OPERATOR_DA_INPUT_BYTES,
            "journal operator DA input exceeds {MAX_JOURNALED_OPERATOR_DA_INPUT_BYTES} bytes"
        );
        anyhow::ensure!(
            self.batch_info.commit_info.edge_da_refs_input.len()
                <= MAX_JOURNALED_EDGE_DA_REFS_INPUT_BYTES,
            "journal edge DA refs input exceeds {MAX_JOURNALED_EDGE_DA_REFS_INPUT_BYTES} bytes"
        );
        anyhow::ensure!(
            self.batch_info.protocol_version == ProtocolSemanticVersion::new(0, 32, 0),
            "durable journal protocol version must be 0.32.0"
        );
...
#[derive(Debug)]
struct JournalIndex {
    entries: BTreeMap<u64, JournalIndexEntry>,
    total_bytes: u64,
    record_limit: usize,
    byte_limit: u64,
}
...
impl JournalIndex {
    fn ensure_additional_capacity(&self, additional_bytes: u64) -> anyhow::Result<()> {
        anyhow::ensure!(
            self.entries.len() < self.record_limit,
            "SNARK journal cannot contain more than {} durable recovery ranges",
            self.record_limit
        );
        let required_bytes = self
            .total_bytes
            .checked_add(additional_bytes)
            .context("SNARK journal aggregate byte count overflow")?;
        anyhow::ensure!(
            required_bytes <= self.byte_limit,
            "SNARK journal would require {required_bytes} durable bytes; maximum is {} bytes",
            self.byte_limit
        );
        Ok(())
    }

    fn insert(&mut self, batch_from: u64, entry: JournalIndexEntry) {
        self.total_bytes = self
            .total_bytes
            .checked_add(entry.serialized_bytes)
            .expect("journal capacity was checked before insertion");
        assert!(self.entries.insert(batch_from, entry).is_none());
    }

    fn remove(&mut self, batch_from: u64) -> Option<JournalIndexEntry> {
        let entry = self.entries.remove(&batch_from)?;
        self.total_bytes = self
            .total_bytes
            .checked_sub(entry.serialized_bytes)
            .expect("journal byte accounting must cover every indexed record");
        Some(entry)
    }
}
...
struct SnarkProofJournalInner {
    directory: PathBuf,
    records: Mutex<JournalIndex>,
    #[cfg(test)]
    fail_next_persist: std::sync::atomic::AtomicBool,
    #[cfg(test)]
    fail_next_persist_after_publication: std::sync::atomic::AtomicBool,
...
impl SnarkProofJournal {
    ...
    /// Malformed or overlapping final records fail startup closed. Transaction temps are never
    /// authoritative because the API cannot return 204 before canonical final publication.
    pub(crate) async fn open(
        proof_storage_path: &Path,
    ) -> anyhow::Result<(Self, mpsc::UnboundedReceiver<String>)> {
        Self::open_with_limits(
            proof_storage_path,
            MAX_STARTUP_RECOVERY_RANGES,
    ...
    #[cfg(test)]
    async fn open_with_record_limit(
        proof_storage_path: &Path,
        record_limit: usize,
    ) -> anyhow::Result<(Self, mpsc::UnboundedReceiver<String>)> {
        Self::open_with_limits(proof_storage_path, record_limit, MAX_JOURNAL_TOTAL_BYTES).await
    }

    // SYSCOIN: Enforce both recovery-plan cardinality and aggregate recovery bytes while building
    // the startup index, before durable JSON can drive deserialization or retained replay state.
    async fn open_with_limits(
        ...
        remove_transaction_temps(&directory).await?;

        let mut records = JournalIndex {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            anyhow::ensure!(
                records.entries.len() < record_limit,
                "SNARK journal contains more than {record_limit} durable recovery ranges"
            );
            let path = entry.path();
            let bytes = read_bounded_journal_file(&path).await?;
            let serialized_bytes =
                u64::try_from(bytes.len()).context("durable SNARK journal length exceeds u64")?;
            records.ensure_additional_capacity(serialized_bytes)?;
            ...
            ensure_no_overlap(&records.entries, record.batch_from, record.batch_to)?;
            records.insert(
                record.batch_from,
                JournalIndexEntry {
                    key,
        ...
        Ok((
            Self {
                inner: Arc::new(SnarkProofJournalInner {
                    ...
                    #[cfg(test)]
                    fail_next_persist_after_publication: std::sync::atomic::AtomicBool::new(false),
    ...
    pub(super) async fn record_count(&self) -> usize {
        self.inner.records.lock().await.entries.len()
    }
    ...
    #[cfg(test)]
    fn fail_next_persist_after_publication_for_test(&self) {
        self.inner
            .fail_next_persist_after_publication
    ...
    pub(super) async fn persist(
        ...
        let serialized_bytes = u64::try_from(serialized.len())
            .context("serialized durable SNARK journal length exceeds u64")?;

        let mut records = self.inner.records.lock().await;
        if let Some(existing) = records.entries.get(&batch_from) {
            anyhow::ensure!(
            ...
            anyhow::ensure!(
                existing.serialized_bytes == serialized_bytes,
                "durable SNARK journal byte accounting mismatch for range {batch_from}-{batch_to}"
            );
            // SYSCOIN: This also repairs the durability boundary after a prior publication reached
            // the canonical name but its directory fsync failed. The failed attempt is indexed for
            // live byte/count accounting, yet no retry is accepted until the directory sync works.
            sync_directory(&self.inner.directory).await?;
            return Ok(record.into_journaled(key));
        }
        ensure_no_overlap(&records.entries, batch_from, batch_to)?;
        ...
        // SYSCOIN: Reserve both count and bytes while holding the same index lock that serializes
        // publication. A live process can therefore never fsync a record that its next startup
        // would reject solely for exceeding the journal recovery limits.
        records.ensure_additional_capacity(serialized_bytes)?;
        #[cfg(test)]
        let fail_after_publication = self
            .inner
            .fail_next_persist_after_publication
            .swap(false, Ordering::SeqCst);
        #[cfg(not(test))]
        let fail_after_publication = false;
        if let Err(error) = durable_publish_new(
        ...
        .await
        {
            // Publication can fail after the canonical hard link exists (for example, on the
            // directory fsync). Account an identical surviving final immediately so later live
            // writes cannot consume the same recovery budget again.
            if fs::try_exists(&final_path).await? {
                let existing_bytes = read_bounded_journal_file(&final_path).await?;
                anyhow::ensure!(
                    existing_bytes == serialized,
                    "conflicting durable SNARK journal content after failed publication for range {batch_from}-{batch_to}"
                );
                records.insert(
                    batch_from,
                    JournalIndexEntry {
    ...
    async fn remove_confirmed_from(
        ...
        let Some(indexed) = records.entries.get(&batch_from) else {
...
impl DurableSnarkRecordV2 {
    ...
    fn into_record(self) -> anyhow::Result<DurableSnarkRecord> {
        ...
        anyhow::ensure!(
            (MIN_JOURNALED_FRIS..=MAX_JOURNALED_FRIS).contains(&self.batches.len()),
...
fn decode_record(path: &Path, bytes: &[u8]) -> anyhow::Result<DurableSnarkRecord> {
...
async fn durable_publish_new(
    ...
    if fail_after_publication {
        anyhow::bail!("injected durable SNARK journal failure after canonical publication");
    }
    #[cfg(not(test))]
    let _ = fail_after_publication;
...
async fn cleanup_unpublished_temp(directory: &Path, temporary_path: &Path) {
    ...
    // proof copies. Failure is safe: startup unlinks every unacknowledged transaction temp.
    if fs::remove_file(temporary_path).await.is_ok() {
        let _ = sync_directory(directory).await;
    }
}

// SYSCOIN: Under the exclusive journal lock, the canonical final name is the sole durable authority.
// A matching temp beside a final is only a stale hard-link alias; one without a final was never
// acknowledged. Remove both cases without retaining unbounded forensic proof copies.
async fn remove_transaction_temps(directory: &Path) -> anyhow::Result<()> {
    let mut entries = fs::read_dir(directory).await?;
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let path = entry.path();
        verify_owner_only_regular_file(&path).await?;
        fs::remove_file(&path).await?;
        removed_any = true;
        tracing::info!(
            path = path.display().to_string(),
            "removed unacknowledged SNARK journal transaction temp"
        );
    }
    if removed_any {
        sync_directory(directory).await?;
...
mod tests {
    ...
    fn serialized_marker_record_len(from: u64, to: u64) -> anyhow::Result<u64> {
        let record = DurableSnarkRecord {
            format_version: JOURNAL_FORMAT_VERSION,
            batch_from: from,
            batch_to: to,
        ...
        Ok(u64::try_from(serialize_record(&record)?.len())?)
    }
    ...
    -> anyhow::Result<()> {
        ...
        assert_eq!(reopened.inner.records.lock().await.entries.len(), 1);
        ...
        tokio::time::timeout(Duration::from_secs(2), async {
            loop {
                if reopened.inner.records.lock().await.entries.is_empty() {
                    break;
                }
        ...
        assert!(after_ack.inner.records.lock().await.entries.is_empty());
        Ok(())
    }
    ...
    async fn identical_retry_is_idempotent_but_overlap_is_rejected() -> anyhow::Result<()> {
        ...
        journal
            .persist(marker_batches(1, 2), real_wrapper())
            .await?;
        assert_eq!(journal.inner.records.lock().await.entries.len(), 1);
    ...
    #[tokio::test]
    async fn restart_rejects_journal_record_count_before_recovery_allocation() -> anyhow::Result<()>
    {
        let temp = TempDir::new()?;
        let (journal, confirmations) = SnarkProofJournal::open(temp.path()).await?;
        journal
            ...
            .await?;
        journal
            .persist(marker_batches(3, 4), real_wrapper())
            .await?;
        drop(confirmations);
        drop(journal);

        let error = SnarkProofJournal::open_with_record_limit(temp.path(), 1)
            .await
            .unwrap_err();
        assert!(
            format!("{error:#}")
                .contains("SNARK journal contains more than 1 durable recovery ranges")
        );
        Ok(())
    }

    #[tokio::test]
    async fn live_journal_enforces_count_before_publishing_unrestartable_record()
    -> anyhow::Result<()> {
        let temp = TempDir::new()?;
        let (journal, confirmations) =
            SnarkProofJournal::open_with_limits(temp.path(), 1, MAX_JOURNAL_TOTAL_BYTES).await?;
        journal
            .persist(marker_batches(1, 2), real_wrapper())
            .await?;
        // Same-key retries consume no additional count budget even at the exact ceiling.
        journal
            .persist(marker_batches(1, 2), real_wrapper())
            .await?;
        ...
        assert!(
            error
                .to_string()
                .contains("more than 1 durable recovery ranges")
        );
        assert_eq!(journal.record_count().await, 1);
        assert!(!fs::try_exists(temp.path().join("snark_journal").join(journal_key(3, 4))).await?);

        drop(confirmations);
        drop(journal);
        let (_reopened, _confirmations) =
            SnarkProofJournal::open_with_limits(temp.path(), 1, MAX_JOURNAL_TOTAL_BYTES).await?;
        Ok(())
    }

    #[tokio::test]
    async fn aggregate_journal_byte_limit_is_exact_live_and_on_restart() -> anyhow::Result<()> {
        let first_len = serialized_marker_record_len(1, 2)?;
        let second_len = serialized_marker_record_len(3, 4)?;

        let live_temp = TempDir::new()?;
        let (live, live_confirmations) =
            SnarkProofJournal::open_with_limits(live_temp.path(), 2, first_len).await?;
        live.persist(marker_batches(1, 2), real_wrapper()).await?;
        // Same-key retries consume no additional byte budget at the exact ceiling.
        live.persist(marker_batches(1, 2), real_wrapper()).await?;
        assert_eq!(live.inner.records.lock().await.total_bytes, first_len);
        let error = live
            .persist(marker_batches(3, 4), real_wrapper())
            .await
            .unwrap_err();
        assert!(error.to_string().contains("durable bytes; maximum is"));
        assert_eq!(live.record_count().await, 1);
        drop(live_confirmations);
        drop(live);

        let restart_temp = TempDir::new()?;
        let exact_limit = first_len.checked_add(second_len).unwrap();
        let (journal, confirmations) =
            SnarkProofJournal::open_with_limits(restart_temp.path(), 2, exact_limit).await?;
        journal
            .persist(marker_batches(1, 2), real_wrapper())
            .await?;
        journal
            .persist(marker_batches(3, 4), real_wrapper())
            .await?;
        assert_eq!(journal.inner.records.lock().await.total_bytes, exact_limit);
        drop(confirmations);
        drop(journal);

        let (exact, exact_confirmations) =
            SnarkProofJournal::open_with_limits(restart_temp.path(), 2, exact_limit).await?;
        assert_eq!(exact.inner.records.lock().await.total_bytes, exact_limit);
        drop(exact_confirmations);
        drop(exact);

        let error = SnarkProofJournal::open_with_limits(restart_temp.path(), 2, exact_limit - 1)
            .await
            .unwrap_err();
        assert!(error.to_string().contains("durable bytes; maximum is"));
        Ok(())
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn identical_unindexed_final_is_charged_before_more_live_publication()
    -> anyhow::Result<()> {
        let temp = TempDir::new()?;
        let record = DurableSnarkRecord {
            format_version: JOURNAL_FORMAT_VERSION,
        ...
        let (journal, confirmations) =
            SnarkProofJournal::open_with_limits(temp.path(), 2, byte_limit).await?;
        let final_path = temp.path().join("snark_journal").join(journal_key(1, 2));
        let mut options = fs::OpenOptions::new();
        options.write(true).create_new(true).mode(0o600);
        let mut file = options.open(&final_path).await?;
        ...
        journal
            .persist(marker_batches(1, 2), real_wrapper())
            .await?;
        assert_eq!(journal.inner.records.lock().await.total_bytes, byte_limit);
        let error = journal
            .persist(marker_batches(3, 4), real_wrapper())
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn post_publication_failure_is_accounted_and_retry_repairs_durability()
    -> anyhow::Result<()> {
        let temp = TempDir::new()?;
        let byte_limit = serialized_marker_record_len(1, 2)?;
        let (journal, confirmations) =
            SnarkProofJournal::open_with_limits(temp.path(), 2, byte_limit).await?;
        journal.fail_next_persist_after_publication_for_test();
        ...
        assert!(
            error
                .to_string()
                .contains("failure after canonical publication")
        );
        assert_eq!(journal.record_count().await, 1);
        assert_eq!(journal.inner.records.lock().await.total_bytes, byte_limit);
        assert!(fs::try_exists(temp.path().join("snark_journal").join(journal_key(1, 2))).await?);
        ...
        // The same-key retry re-syncs the directory before reporting durable acceptance.
        journal
            .persist(marker_batches(1, 2), real_wrapper())
            .await?;
        drop(confirmations);
    ...
    // SYSCOIN: Owner-only journal files are read under the 256 MiB file cap, then the typed V2
    // decoder enforces every protocol-sized collection before canonical recovery can retain it.
    #[test]
    fn typed_v2_decode_enforces_dynamic_protocol_bounds() -> anyhow::Result<()> {
        let record = DurableSnarkRecord {
            format_version: JOURNAL_FORMAT_VERSION,
            batch_from: 1,
            batch_to: 2,
        ...
        let durable = || DurableSnarkRecordV2::from_record(&record).unwrap();
        let decode_error = |record: DurableSnarkRecordV2| {
            record
                .into_record()
                .err()
                .expect("invalid journal fixture must fail")
                .to_string()
        };

        let mut invalid = durable();
        invalid.batches.truncate(1);
        assert!(decode_error(invalid).contains("aggregate count"));

        let mut invalid = durable();
        invalid.batches[0].batch.batch_info.protocol_version =
            ProtocolSemanticVersion::new(0, 31, 0);
        assert!(decode_error(invalid).contains("protocol version"));
        ...
        invalid.batches[0]
            ...
            .batch_info
            .commit_info
            .operator_da_input = vec![0; MAX_JOURNALED_OPERATOR_DA_INPUT_BYTES + 1];
        assert!(decode_error(invalid).contains("operator DA input"));
        ...
        invalid.batches[0]
            ...
            .batch_info
            .commit_info
            .edge_da_refs_input = vec![0; MAX_JOURNALED_EDGE_DA_REFS_INPUT_BYTES + 1];
        assert!(decode_error(invalid).contains("edge DA refs input"));

        let mut invalid = durable();
        invalid.batches[0].batch.message_lengths = vec![0; MAX_JOURNALED_MESSAGES_PER_BATCH + 1];
        assert!(decode_error(invalid).contains("messages exceed"));

        let mut invalid = durable();
        invalid.batches[0].batch.message_lengths =
            vec![(MAX_JOURNALED_MESSAGE_BYTES_PER_BATCH + 1) as u32];
        assert!(decode_error(invalid).contains("message bytes exceed"));
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn unacknowledged_transaction_temp_is_unlinked() -> anyhow::Result<()> {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn restart_unlinks_all_unacknowledged_transaction_temps() -> anyhow::Result<()> {
        use std::os::unix::fs::PermissionsExt as _;

        let temp = TempDir::new()?;
        let directory = temp.path().join("snark_journal");
        fs::create_dir_all(&directory).await?;
        let interrupted = ["first", "second"].map(|name| {
            directory.join(format!("{JOURNAL_TEMP_PREFIX}{name}{JOURNAL_TEMP_SUFFIX}"))
        });
        for path in &interrupted {
            fs::write(path, b"unacknowledged").await?;
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn published_transaction_temp_alias_is_unlinked_but_final_remains() -> anyhow::Result<()>
    {
        let temp = TempDir::new()?;
        let directory = temp.path().join("snark_journal");
        fs::create_dir_all(&directory).await?;
