...
verify_semantics() {
  # SYSCOIN: Every surviving source/config file touched downstream remains visibly
  # attributable after a rebase. Generated locks, docs, fixtures, and deleted files
  # are bound by the exact patch/tree attestations instead.
  local tagged_path
  while IFS= read -r tagged_path; do
    case "${tagged_path}" in
    *.rs | *.toml | *.sh)
      [[ -f "${ZKSYNC_OS_PATH}/${tagged_path}" ]] || continue
