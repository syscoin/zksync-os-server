use crate::{Bridgehub, IChainAssetHandler, ZkChain};
...
async fn find_settlement_layer_intervals(
    ...
    // SYSCOIN: The pinned V32 ChainAssetHandler must expose its complete migration history.
    // Treating a missing getter as direct-L1 can erase prior Gateway intervals after a return.
    let total_migrations: u64 = cah
        .migrationNumber(U256::from(chain_id))
        .call()
        .await
        .map_err(|error| anyhow::Error::new(error).context("failed to fetch migrationNumber"))?
        .try_into()
        .map_err(|error| anyhow::anyhow!("migrationNumber overflow: {error}"))?;
