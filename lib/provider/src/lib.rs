...
pub struct ProviderCapabilities {
    ...
    /// client detection with a chain-id heuristic as the final fallback.
...
impl ProviderCapabilities {
    ...
    async fn detect(provider: &impl Provider<Ethereum>) -> Self {
        ...
        // fork is Osaka or later, regardless of what the client software could support.
        let supports_eip7594 = match Self::probe_eth_config_eip7594(provider).await {
            ...
            // SYSCOIN: gateway scripts use Anvil with custom chain IDs, so the upstream chain-id
            // fallback alone would incorrectly select EIP-7594. Detect Anvil by client version
            // first, while retaining upstream's chain-id check for every non-Anvil or failed
            // client-version probe.
            None => match provider.get_client_version().await {
                Ok(version) if version.to_lowercase().contains("anvil") => false,
                Err(err) => {
...
mod tests {
    ...
    async fn uses_get_header_when_supported() {
        ...
        // finalized supported; eth_config unavailable; client version -> non-anvil.
    ...
    #[tokio::test]
    async fn unrecognized_eth_config_falls_back_to_client_and_chain_id() {
        let asserter = Asserter::new();
        // SYSCOIN: an unrecognized eth_config response falls back to client detection and then
        // upstream's chain-id heuristic.
        asserter.push_success(&header_with_number(1));
        asserter.push_success(&header_with_number(1));
    ...
    #[tokio::test]
    async fn non_anvil_default_dev_chain_id_disables_eip7594() {
        let asserter = Asserter::new();
        asserter.push_success(&header_with_number(1));
        asserter.push_success(&header_with_number(1));
        asserter.push_failure(unsupported_method());
    ...
    async fn anvil_client_version_disables_eip7594() {
        let asserter = Asserter::new();
        // No chain-id response is queued: Anvil is disabled even when scripts assign a custom ID.
        asserter.push_success(&header_with_number(1));
        asserter.push_success(&header_with_number(1));
        asserter.push_failure(unsupported_method());
