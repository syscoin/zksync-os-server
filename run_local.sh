...
if [ -n "$LOGS_DIR" ]; then
    ...
    anvil --load-state "$L1_STATE_FILE" --port 8545 --block-time 1 --mixed-mining > "$ANVIL_LOG_FILE" 2>&1 &
    echo -e "${GREEN}Anvil logs: $ANVIL_LOG_FILE${NC}"
else
    anvil --load-state "$L1_STATE_FILE" --port 8545 --block-time 1 --mixed-mining > /dev/null 2>&1 &
fi
ANVIL_PID=$!
PIDS+=($ANVIL_PID)
