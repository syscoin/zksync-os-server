...
use crate::prover_api::{
    MAX_FRI_PICK_RESPONSE_BYTES, fri_job_manager::FriJobManager, proof_storage::ProofStorage,
    prover_server::v1::v1_routes, snark_job_manager::SnarkJobManager,
};
...
// SYSCOIN: Preserve three-way FRI fleet distribution while bounding simultaneous witness clones,
// encodings, compression, and transfers independently of tokenless diagnostics.
const FRI_PICK_CONCURRENCY: usize = 3;
// SYSCOIN: Serialize authenticated aggregate assignment, encoding, compression, and the trusted
// proxy drain without limiting how many SNARK workers may prove already-delivered ranges.
const SNARK_PICK_CONCURRENCY: usize = 1;
// SYSCOIN: A tokenless diagnostic peek can repeatedly clone and encode the same full witness.
// Serialize one at a time and retain the permit until its bounded response body is released.
const FRI_PEEK_CONCURRENCY: usize = 1;
// SYSCOIN: Bound aggregate diagnostics independently so a large SNARK peek cannot starve FRI.
const SNARK_PEEK_CONCURRENCY: usize = 1;
...
// SYSCOIN: A remote picker may retry through another endpoint only when this application marker
// proves the response was produced before any manager lease existed.
pub(super) const PROVER_PICK_OUTCOME_HEADER: &str = "x-syscoin-prover-pick-outcome";
pub(super) const PROVER_PICK_OUTCOME_UNLEASED: &str = "unleased";
...
pub(in crate::prover_api::prover_server) struct AppState {
    ...
    fri_pick_slots: Arc<Semaphore>,
    snark_pick_slots: Arc<Semaphore>,
    fri_peek_slots: Arc<Semaphore>,
    snark_peek_slots: Arc<Semaphore>,
    max_fri_response_bytes: usize,
...
pub async fn run(
    ...
    let app_state = AppState {
        ...
        proof_storage,
        max_fris_per_snark,
        fri_pick_slots: Arc::new(Semaphore::new(FRI_PICK_CONCURRENCY)),
        snark_pick_slots: Arc::new(Semaphore::new(SNARK_PICK_CONCURRENCY)),
        fri_peek_slots: Arc::new(Semaphore::new(FRI_PEEK_CONCURRENCY)),
        snark_peek_slots: Arc::new(Semaphore::new(SNARK_PEEK_CONCURRENCY)),
        max_fri_response_bytes: MAX_FRI_PICK_RESPONSE_BYTES,
    ...
    // SYSCOIN: Preserve loopback as a runtime invariant even if a future caller bypasses config
    // validation. Remote workers must enter through the generated fully buffering HTTPS proxy.
    assert!(
        addr.ip().is_loopback(),
        "prover API listener must be bound to loopback"
    );
