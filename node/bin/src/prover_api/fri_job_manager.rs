...
use crate::prover_api::prover_job_map::{
    BeginSubmissionError, LeasedJob, ProverJobMap, SubmissionLease,
};
...
impl FriJobManager {
    ...
    ///
    /// SYSCOIN: The returned job always carries the opaque capability for its atomic assignment.
    ...
    pub async fn submit_fake_proof(
        ...
        let mut completed = submission
            .complete(ProverType::Fake, prover_id)
            .await
    ...
    fn persist_and_enqueue_accepted_proof(
        ...
        tokio::spawn(async move {
            ...
            let Some(mut removed_jobs) = submission.complete(ProverType::Real, &prover_id).await
            else {
            ...
            let removed_job = removed_jobs
                .pop()
                .expect("single FRI submission completion must remove one job");
...
mod tests {
    ...
    async fn begin_test_submission(
        ...
        let picked = manager
            .pick_next_job(Duration::ZERO, prover_id.to_owned(), None)
            .await
    ...
    -> anyhow::Result<()> {
        ...
        let picked = manager
            .pick_next_job(Duration::ZERO, "victim-display-id".to_owned(), None)
            .await
        ...
        let reassigned = manager
            .pick_next_job(Duration::ZERO, "new-display-id".to_owned(), None)
            .await
    ...
    -> anyhow::Result<()> {
        ...
        manager.add_job(dummy_input_batch(1)).await;
        let picked = manager
            .pick_next_job(Duration::ZERO, "display-a".to_owned(), None)
            .await
            .expect("FRI job must be assigned");
        ...
        retry.release().await;
        let reassigned = manager
            .pick_next_job(Duration::ZERO, "display-c".to_owned(), None)
            .await
    ...
    async fn stale_fake_fri_cannot_consume_reassigned_real_lease() -> anyhow::Result<()> {
        ...
        let real = manager
            .pick_next_job(Duration::ZERO, "real-prover".to_owned(), None)
            .await
