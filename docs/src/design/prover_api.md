...
| `Drainable` | If enabled, only the loopback prover listener opens behind the buffering HTTPS proxy. | Remains HTTP 503. | Journal replay and its supervised forwarding/reaping paths are live, so workers may safely drain bounded proving work while FRI recovery continues. |
...
Each durable SNARK record is limited to 256 MiB, while the complete unconfirmed journal is limited
...
outage for a restart working set that remains safe on the 64-GiB production hosts.
