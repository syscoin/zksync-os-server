# Shared preparation for builds that apply the Syscoin zksync-os patch to the
# immutable upstream revision selected by this server workspace's Cargo.lock.
# Callers must source gateway-launch/_common.sh first and set
# ZKSYNC_OS_SERVER_PATH, GATEWAY_DIR, WORKSPACE_NAME, and ZKSYNC_OS_GIT_URL.
...
if cargo_lock.exists():
    ...
    def local_source(match: re.Match[str]) -> str:
        revision = f"#{os_rev}" if match.group("revision") else ""
        return f"git+{os_git_url}?tag={os_tag}{revision}"

    lock_text, count = source_re.subn(local_source, lock_text)
    if count == 0:
