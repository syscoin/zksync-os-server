...
pub enum SystemTxType {
    ...
    /// chain id and the migration number. SYSCOIN: the migration number is `u64::MAX` for the
    /// protocol-upgrade / fresh-chain placeholder, which initializes the field but is not a migration.
