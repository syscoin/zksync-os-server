...
library P256Verifier {
    ...
    function isValid(bytes32 digest, bytes32 r, bytes32 s, bytes32 x, bytes32 y) internal view returns (bool) {
        ...
        // Fail closed: a missing or failing precompile must count as an invalid signature
        // so account validation never reverts on environments without P-256 support.
