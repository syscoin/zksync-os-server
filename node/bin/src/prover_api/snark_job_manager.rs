...
use crate::prover_api::prover_job_map::{
    BeginSubmissionError, JobEntry, JobMapCapacityExceeded, ProverJobMap, SnarkJobAdmission,
    SnarkJobEligibility, SnarkJobPick, SnarkOwnershipCompletion, SnarkOwnershipSeedError,
    StartupRecoveryBoundaryError, StartupRecoveryPlan,
};
...
impl SnarkJobManager {
    ...
    pub async fn submit_proof(
        ...
        if let Some(journal) = &self.journal {
            ...
            let handoff = tokio::spawn(async move {
                ...
                match submission
                    .complete_with_snark_ownership(ProverType::Real, &prover_id)
                    .await
                {
    ...
    async fn complete_fake_leased_jobs(
        ...
        let submission = self
            ...
            .ok()?;
        match submission
            .complete_with_snark_ownership(ProverType::Fake, prover_id)
            .await
        {
